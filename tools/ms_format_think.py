...
output_file = './toolsciverifier/sciprm-bench/train_conv_think.jsonl'
...
SYSTEM_PROMPT = (
    ...
    "Response format: First provide your thought process inside <think> </think> tags, then output 'Valid' or 'Invalid'."
)
...
def process_file(input_path, output_path):
    ...
    with open(input_path, 'r', encoding='utf-8') as fin, \
        ...
        for line in fin:
            ...
            for i, step in enumerate(reasoning_chain):
                ...
                # --- 构建 Assistant Content (带 <think>) ---
                is_valid = step.get('step_label', False)
                verdict = "Valid" if is_valid else "Invalid"
                
                # 获取 label_reason 作为思考过程
                think_content = step.get('label_reason', 'No specific reason provided.')
                
                # 组合最终输出：<think>...</think> + Verdict
    ...
    print(f"Conversion complete. Processed {count} items. Saved to {output_path}")
